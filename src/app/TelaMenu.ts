...
export class TelaMenu implements Tela {
  ...
  private card(topico: Topico): string {
    const comandos: string = topico.licoes.length > 0
      ? Array.from(new Set(topico.licoes.map((l) => l.comando.split(/[ /]/)[0])))
        .filter((nome: string) => Shell.obterRegistro().obter(nome) !== undefined)
        .map((nome: string) => '<code>' + nome + '</code>').join(' ')
